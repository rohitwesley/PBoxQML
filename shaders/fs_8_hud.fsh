...
//    else {
        ...
        int rez = s*t;
        vec2 id;
        //id = vec2(iFrame%rez,iFrame/rez);
        //id = vec2(iTime%rez,iTime/rez);
        if(iHUD.z>0) id = vec2(int(iHUD.x+36)%rez,int(iHUD.x+16)/rez);
        else id = vec2(int(iHUD.x)%rez,int(iHUD.x)/rez);
        //vec2 id = vec2(0,1);
        color = texture(fragTexture, vec2(id.x/s+uv.x/s,id.y/t+uv.y/t));
