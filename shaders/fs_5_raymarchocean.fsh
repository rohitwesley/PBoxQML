...
#define EPSILON_NRM 0.0001
//#define EPSILON_NRM (0.0001 / iResolution.x)
...
#define SEA_TIME (1.0 + ((iTime+iFrame)*0.01) * SEA_SPEED)
...
void main()
{
    ...
    if (selected) {
        ...
        vec2 uv = frag_uv.xy;// / iResolution.xy;
        ...
        // post
        color = vec4(pow(oceanframe,vec3(0.75)), 1.0);// * texture(fragTexture, uvIndexText);
