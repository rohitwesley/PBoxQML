...
QString SqlModel::sender() const
{
    return m_sender;
}

void SqlModel::setSender(const QString &sender)
{
    if (sender == m_sender)
        return;
...
QString SqlModel::recipient() const
{
    return m_recipient;
}

void SqlModel::setRecipient(const QString &recipient)
{
...
void SqlModel::sendMessage(const QString &recipient, const QString &message)
{
...
void SqlModel::refreshMessage()
{
    submitAll();
}

void SqlModel::clearMessages(const QString &recipient)
{
    QSqlQuery query;
    QString queryString = QString::fromLatin1(
           "DELETE FROM Conversations WHERE (recipient = '%1' AND author = '%2') OR (recipient = '%2' AND author='%1')").arg(recipient,m_sender);
